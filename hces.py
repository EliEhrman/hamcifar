...
This module tries to adapt this to Evolution Strategy instead
"""
...
def mutate_int_to_bits(l_sm_int_to_bits):
	...
	for isel, sel_mat in enumerate(l_sm_int_to_bits):
		if random.random() < c_rnd_asex:
			incr_vals = np.random.randint(-1, 2, (c_rgb_dims * c_bits_per_src_val, c_img_x, c_img_y), dtype=np.int16)
			sel_mat[:, :, :] = (np.clip(incr_vals + sel_mat, 0, iui8.max)).astype(np.uint8)
