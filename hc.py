...
c_int_to_bit_mut_rate = 7.0 / (c_rgb_dims * c_bits_per_src_val * 32 * 32)  # 0.0 -> 1.0. 1.0 means change them all, 0.0 means change none
...
def conv2d(img, W):
	...
	new_layer = np.zeros((w, h, n, num_o_channels), dtype=np.uint8)
	for sx in range(w):
		for sy in range(h):
			...
			for iop in range(num_o_channels):
				new_pix = np.take(slice_r, W[iop][0], axis=1)
...
def run_one_set(numrecs, nd_img_r_data, l_labels, train_limit, l_sms, b_real=False):
	...
	for ilayer in range(len(c_l_num_channels) - 1):
		layer = conv2d(mp_layer, l_sms[ilayer+1])
		mp_layer = maxpool(layer)
...
def mutate_int_to_bits(l_sm_int_to_bits):
	for isel, sel_mat in enumerate(l_sm_int_to_bits):
		if random.random() < c_rnd_asex:
			new_vals = np.random.randint(0, 255, (c_rgb_dims * c_bits_per_src_val, 32, 32), dtype=np.uint8)
			new_mask = np.random.rand(c_rgb_dims * c_bits_per_src_val, 32, 32)
			sel_mat[:,:,:] = np.where(new_mask < c_int_to_bit_mut_rate, new_vals, sel_mat)
...
def init():
	...
	nd_img_data = nd_src.reshape(-1, 3, 32, 32)
...
for iW in range(c_num_Ws):
	...
	for ilayer in range(len(c_l_num_channels)):
		...
		for iop in range(c_l_num_channels[ilayer]):
			# num_samps = random.randint(c_min_bits, c_max_bits)
			num_samps = c_min_bits + 1 # try intializing with low nu sampls. Equivalent to init of W with small weights
			# W.append([[random.randint(0, (c_l_num_channels[ilayer-1]*c_fsize*c_fsize)-1) for _ in range(num_samps)],
			# 		  random.randint(1, num_samps)])
			W.append([[random.randint(0, (c_l_num_channels[ilayer - 1] * c_fsize * c_fsize) - 1) for _ in range(num_samps)],
					  2])
		# l_layer_Ws.append(W)
		l_l_layer_Ws[ilayer-1].append(W)

	sm_int_to_bits = np.random.randint(0, 255, (c_rgb_dims * c_bits_per_src_val, 32, 32), dtype=np.uint8)
